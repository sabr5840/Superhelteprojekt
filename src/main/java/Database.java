...
public class Database {
    ...
    public ArrayList <Superhelte> searchFor(String searchTerm){
        ArrayList<Superhelte> fundetResultater = new ArrayList<>();
        for (Superhelte superhelte : superhelte){
            String navn = superhelte.getNavn();
            if (superhelte.getNavn().equals(searchTerm)) {
                fundetResultater.add(superhelte);
            }
    }
        return fundetResultater;
